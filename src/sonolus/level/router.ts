...
import { AuthRequest, isAuth } from "../../api/middleware/isAuth.js";
...
const isChartOwner = async (req: AuthRequest, levelName: string) => {
    ...
    if (!level) {
        console.log(`デバッグ: 譜面が見つかりません: ${levelName}`);
        return false;
    }

    if (req.user.role === 'admin' || req.user.role === 'moderator') {
        console.log('デバッグ: 管理者/モデレーター権限で許可');
        return true;
    }

    const authorHandle = level.author?.ja?.split('#')[1] || "";
    console.log(`デバッグ: 抽出されたauthorHandle: "${authorHandle}"`);
    
    if (!authorHandle) {
        console.log('デバッグ: authorHandleが空のため権限なし');
        return false;
    }

    const userHandle = req.user.sonolusProfile?.handle;
    const authorHandleInt = parseInt(authorHandle);
    const userHandleInt = typeof userHandle === 'string' ? parseInt(userHandle) : userHandle;
    
    
    if (userHandleInt === authorHandleInt) {
        return true;
    }

    if (level.meta?.anonymous?.isAnonymous) {
        const originalHandle = Number(level.meta.anonymous.original_handle);
        if (userHandleInt === originalHandle) {
            console.log('デバッグ: 匿名投稿の元投稿者として許可');
            return true;
        }
    }
    ...
    if (level.meta?.collaboration?.iscollaboration && level.meta.collaboration.members) {
        const isCollaborator = level.meta.collaboration.members.some(
            (member) => member.handle != null && Number(member.handle) === userHandleInt
        );
        if (isCollaborator) {
            console.log('デバッグ: 合作メンバーとして許可');
            return true;
        }
    }
    ...
    sonolus.router.patch('/api/chart/edit/:id',
        ...
        async (req: AuthRequest, res, next) => {
            try {
                ...
                if (!existingLevel) {
                    res.status(404).json({ error: '指定された譜面が見つかりません' });
                    return;
                }

                // 権限チェックを追加
                const hasPermission = await isChartOwner(req, id);
                if (!hasPermission) {
                    res.status(403).json({ error: 'この譜面を編集する権限がありません' });
                    return;
                }
                ...
                // キャッシュをクリア
                await CacheInvalidation.clearChartCaches(id);
...
export const deleteLevel = () => {
    sonolus.router.delete('/api/chart/delete/:id', isAuth, async (req: AuthRequest, res, next) => {
        try {
            const { id } = req.params;
            ...
            const hasPermission = await isChartOwner(req, id);
            if (!hasPermission) {
                res.status(403).json({ error: 'この譜面を削除する権限がありません' });
                return;
            }
